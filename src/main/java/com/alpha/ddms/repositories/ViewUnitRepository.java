...
public interface ViewUnitRepository extends JpaRepository<ViewUnit, String> {
    @Query(value = "select * from vw_mst_unit where unit_id=:unitId", nativeQuery = true)
    List<ViewUnit> findByIdUnit(String unitId);
    @Query(value = "select * from vw_mst_unit where dealer_code=:dealerCode and lower(unit_status)=lower(:unitStatus) and lower(unit_series_name) like lower(concat('%',:unitseriesname,'%'))",nativeQuery = true)
